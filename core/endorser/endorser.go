...
func initPBCkeys() {
	dir, _ := os.Getwd()
	endorserLogger.Infof("ws add: pwd is: " + dir)
	...
	g.Rand()
	fmt.Printf("G = %s\n", g)
	endorserLogger.Infof("ws add: end g init")
}
...
func timeEncryption(plaintext []byte, random []byte) []byte {
	...
	endorserLogger.Infof("pkuLeft ag from byte = %s\n", pkuLeft)

	f, _ = ioutil.ReadFile("./userPK_asg.dat")
	pkuRight = pkuRight.SetBytes(f)
	endorserLogger.Infof("pkuRight asg from byte = %s\n", pkuRight)

	f, _ = ioutil.ReadFile("./serPK_g.dat")
	g = g.SetBytes(f)
	endorserLogger.Infof("g from byte = %s\n", g)

	f, _ = ioutil.ReadFile("./serPK_sg.dat")
	pks = pks.SetBytes(f)
	endorserLogger.Infof("sg from byte = %s\n", pks)
	...
	if check {
		endorserLogger.Infof("pass check!\n")
	} else {
		endorserLogger.Infof("check failed!\n")
	}
	// step2: random r
	r.Rand()
	endorserLogger.Infof("r = %s\n", r)
	rg = rg.MulZn(g, r)
	endorserLogger.Infof("rg = %s\n", rg)
	writeBytesToFile(rg.Bytes(), "./rg.dat")

	// rgstr := string(rg.Bytes())
	// fmt.Println("rg str is: " + rgstr)
	// writeBytesToFile([]byte(rgstr), "./rg.dat")

	rasg = rasg.MulZn(pkuRight, r)
	endorserLogger.Infof("rasg = %s\n", rasg)
	...
	endorserLogger.Infof("random = %s\n", string(timeStr))

	h1t = h1t.SetFromHash(timeStr)
	endorserLogger.Infof("H1(t) = %s\n", h1t)

	// step3: compute encryption key
	K = K.Pair(rasg, h1t)
	endorserLogger.Infof("K = %s\n", K)

	// step4: encrypt message
	n := K.BytesLen()
	endorserLogger.Infof("length of H2(k): %d \n", n)
	h2k := K.Bytes()
	endorserLogger.Infof("H2(K) is %s \n", h2k)

	// message := []byte("test message hello world")
	message := plaintext

	endorserLogger.Infof("message is: %s \n", message)
	mLength := len(message)
	endorserLogger.Infof("length of message: %d \n", mLength)
	...
	endorserLogger.Infof("length of ciphertext is: %d \n", ciphtLength)
	...
	endorserLogger.Infof("ciphertext str is: " + ciphertextString)
...
<<<<<<< HEAD
		// endorserLogger.Infof("ws add: txParams.TXSimulator == nil")
...
>>>>>>> 867fbedd06c667ac880ebe82b5a18eddc059ec33
	...
	// endorserLogger.Infof("ws add: input as follows")
	// endorserLogger.Infof(strconv.Itoa(len(chaincodeInput.Args)))
	// for i := 0; i < len(chaincodeInput.Args); i++ {
	// 	endorserLogger.Infof(string(chaincodeInput.Args[i]))
	// }
	funcname := ""
	random := []byte("")
	if len(chaincodeInput.Args) == 4 && string(chaincodeInput.Args[0]) == "TransferAssetWS" {
		funcname = "TransferAssetWS"
		random = chaincodeInput.Args[1]
	}
	...
	if simResult.PvtSimulationResults != nil {
		...
		if funcname == "TransferAssetWS" {
			// initPBCkeys()
			writeBytesToFile(random, "./random.dat")
		}
		//ws add end

		endorserLogger.Infof("ws add: start AssemblePvtRWSet")
		...
		//ws add: remove the following four lines can disable the pdc distribution
		pvtDataWithConfig.EndorsedAt = endorsedAt
		//ws add
		// pvtDataWithConfigCipher.EndorsedAt = endorsedAt
...
// err := proto.Unmarshal(itemCollection[j].Rwset, kvrwset1)
// if err != nil {
// 	endorserLogger.Infof(kvrwset1.Writes[0].Key)
// 	endorserLogger.Infof(string(kvrwset1.Writes[0].Value))
// } else {
// 	endorserLogger.Infof("ws add: Rwset Unmarshal failed")
// }
