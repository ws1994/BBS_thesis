...
func (t *SimpleChaincode) readFile(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var err error
	if len(args) != 6 {
		return shim.Error("00001")
	}
...
/**download*/
func (t *SimpleChaincode) downloadFile(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	var err error
	if len(args) != 8 {
		return shim.Error("00001")
	}
...
//delete ReadFile
func (t *SimpleChaincode) deleteReadFile(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
		return shim.Error("00001")
	}
	...
	go func() {
		for _, file := range files {
			...
			go func() {
				...
				if err != nil {
					fmt.Println("deleteReadFile err is not nil")
					return
				}
				wait.Done()
			}()
...
//delete UploadFile
func (t *SimpleChaincode) deleteUploadFile(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
		return shim.Error("00001")
	}
...
//delete DownloadFile
func (t *SimpleChaincode) deleteDownloadFile(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	if len(args) != 1 {
		return shim.Error("00001")
	}
