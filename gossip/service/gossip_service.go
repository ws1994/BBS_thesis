...
// ws add start
func writeBytesToFile(content []byte, filePath string) {
	// filePath = "./random.dat"
	...
	defer file.Close()
	_ = ioutil.WriteFile(filePath, content, 0666)
}

func timeEncryption(plaintext []byte, random []byte) []byte {
	// *********
	// time encryption
	//**********
	filePath := "./parameter.txt"
	f, _ := ioutil.ReadFile(filePath)
	// endorserLogger.Infof(string(f))
	...
	// *****
	// step0: read keys
	f, _ = ioutil.ReadFile("./userPK_ag.dat")
	pkuLeft = pkuLeft.SetBytes(f)
	// fmt.Printf("pkuLeft ag from byte = %s\n", pkuLeft)
	// logger.Infof("pkuLeft ag from byte = %s\n", pkuLeft)

	f, _ = ioutil.ReadFile("./userPK_asg.dat")
	pkuRight = pkuRight.SetBytes(f)
	// logger.Infof("pkuRight asg from byte = %s\n", pkuRight)

	f, _ = ioutil.ReadFile("./serPK_g.dat")
	g = g.SetBytes(f)
	// logger.Infof("g from byte = %s\n", g)

	f, _ = ioutil.ReadFile("./serPK_sg.dat")
	pks = pks.SetBytes(f)
	// logger.Infof("sg from byte = %s\n", pks)
	...
	if check {
		logger.Infof("pass check!\n")
	} else {
		logger.Infof("check failed!\n")
	}
	// step2: random r
	r.Rand()
	// logger.Infof("r = %s\n", r)
	rg = rg.MulZn(g, r)
	// rgLength := len(rg.Bytes())
	// logger.Infof("length of rg: %d \n", rgLength) // 128
	// logger.Infof("rg = %s\n", rg)
	writeBytesToFile(rg.Bytes(), "./rg.dat")

	// rgstr := string(rg.Bytes())
	// logger.Infof("rg str is: " + rgstr)
	// writeBytesToFile([]byte(rgstr), "./rg.dat")
	...
	// timeStr := randSeq(128)
	// writeBytesToFile([]byte(timeStr), "./random.dat")
	timeStr := random

	// logger.Infof("random = %s\n", string(timeStr))

	h1t = h1t.SetFromHash(timeStr)
	// logger.Infof("H1(t) = %s\n", h1t)
	...
	n := K.BytesLen()
	// logger.Infof("length of H2(k): %d \n", n)
	h2k := K.Bytes()
	// logger.Infof("H2(K) is %s \n", h2k)

	// message := []byte("test message hello world")
	message := plaintext

	logger.Infof("message is: %s \n", message)
	mLength := len(message)
	// logger.Infof("length of message: %d \n", mLength)

	ciphertext := make([]byte, mLength)
	for i := 0; i < mLength; i++ {
		ciphertext[i] = message[i] ^ h2k[i%n]
	}
	// endorserLogger.Infof(ciphertext)
	// ciphertext = []byte{'G', 'O', 'L', 'A', 'N', 'G'}

	// ciphtLength := len(ciphertext)
	// logger.Infof("length of ciphertext is: %d \n", ciphtLength) //142

	writeBytesToFile(ciphertext, "./ciphertext.dat")

	// rgString := string(rg.Bytes())
	// timeString := timeStr
	// ciphertextString := string(ciphertext)
	// logger.Infof("ciphertext str is: " + ciphertextString)

	// ciphertextString = bytes.NewBuffer(ciphertext).String()
	// log.Println("ciphertext str 2 is: " + ciphertextString)
	// fmt.Println([]byte(ciphertextString))

	cipher := append(ciphertext, rg.Bytes()...)
	// cipherLength := len(cipher)
	// logger.Infof("length of cipher: %d \n", cipherLength)
...
func (g *GossipService) DistributePrivateData(channelID string, txID string, privData *tspb.TxPvtReadWriteSetWithConfigInfo, blkHt uint64) error {
	logger.Info("ws add: start to distributePrivateData")
	...
	// logger.Infof("ws add: privData without encryption as follows:")
	// logger.Infof(privData.GetPvtRwset().String())
	...
	//ws add
	pvtDataWithConfig := privData
	// logger.Infof("ws add: pvtDataWithConfig is as follows:")
	// logger.Infof(pvtDataWithConfig.GetPvtRwset().String())
	nspvtSize := len(pvtDataWithConfig.GetPvtRwset().NsPvtRwset)
	// if funcname == "TransferAssetWS" {
	for i := 0; i < nspvtSize; i++ {
		itemNsPvtRwset := pvtDataWithConfig.GetPvtRwset().NsPvtRwset[i]
		// logger.Infof(itemNsPvtRwset.String())

		itemCollection := itemNsPvtRwset.CollectionPvtRwset
		for j := 0; j < len(itemCollection); j++ {
			// logger.Infof("ws add: collection name is:" + itemCollection[j].CollectionName)
			// logger.Infof("ws add: Rwset is:" + string(itemCollection[j].Rwset))

			if itemCollection[j].CollectionName == "assetCollection" {
				random, _ := ioutil.ReadFile("./random.dat")
				// logger.Infof("random = %s\n", random)

				// logger.Infof("timeEncryption start \n")
				rwCiphertext := timeEncryption(itemCollection[j].Rwset, random)
				// logger.Infof("ws add: Rwset ciphertext is:" + string(rwCiphertext))
				pvtDataWithConfig.PvtRwset.NsPvtRwset[i].CollectionPvtRwset[j].Rwset = rwCiphertext
			}
		}
	}
	// logger.Infof("ws add: pvtDataWithConfig after encryption as follows:")
	// logger.Infof(pvtDataWithConfig.GetPvtRwset().String())
	//ws add end

	// ws modify: change privData to pvtDataWithConfig
	// if err := handler.distributor.Distribute(txID, privData, blkHt); err != nil {
	if err := handler.distributor.Distribute(txID, pvtDataWithConfig, blkHt); err != nil {
		err := errors.WithMessagef(err, "failed to distribute private collection, txID %s, channel %s", txID, channelID)
		logger.Error(err)
		return err
	}
	// ws add: move to before Distribute
	// logger.Infof("ws add: privData without encryption as follows:")
	// logger.Infof(privData.GetPvtRwset().String())
	// if err := handler.coordinator.StorePvtData(txID, privData, blkHt); err != nil {
	// 	logger.Error("Failed to store private data into transient store, txID",
